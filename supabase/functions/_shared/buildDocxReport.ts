...
// Mirrors the design tokens in src/index.css — keep in sync if those change.
const COLORS = {
  ink: '17191C',
  sub: '7A7A71',
  accent: 'B38719',
...
function para(opts: any = {}): Paragraph {
  ...
  const textRuns = runs || [new TextRun({ text, size: HALF(size), bold, italics, color, font: FONTS.body })];
...
function sectionHeading(title: string): (Paragraph | Paragraph)[] {
  return [
    new Paragraph({
      ...
      children: [new TextRun({ text: title, size: HALF(15), bold: true, color: COLORS.ink, font: FONTS.heading })],
...
function bodyText(text: string): Paragraph {
  return new Paragraph({
    ...
    children: [new TextRun({ text: text || '', size: HALF(11), color: COLORS.ink, font: FONTS.body })],
  });
}
    ...
    children: headers.map((h) =>
      new TableCell({
        ...
        children: [
          new Paragraph({
            spacing: { after: 0 },
            children: [new TextRun({ text: String(h ?? ''), size: HALF(10), bold: true, color: COLORS.white, font: FONTS.body })],
          }),
        ],
      })
    ),
...
function coverPageChildren(spec: any): (Paragraph | Table)[] {
  ...
  children.push(new Paragraph({
    ...
    spacing: { after: 80 },
    children: [new TextRun({ text: spec.company?.name || 'Company', size: HALF(12), color: COLORS.sub, font: FONTS.body })],
  }));
  ...
  children.push(new Paragraph({
    alignment: AlignmentType.LEFT,
    spacing: { after: 120 },
    children: [new TextRun({ text: spec.modelTitle || 'Report', size: HALF(30), bold: true, color: COLORS.ink, font: FONTS.heading })],
  }));
  ...
  if (spec.subtitle) {
    children.push(new Paragraph({
      alignment: AlignmentType.LEFT,
      spacing: { after: 600 },
      children: [new TextRun({ text: spec.subtitle, size: HALF(13), color: COLORS.accent, font: FONTS.heading })],
    }));
  } else {
...
export async function buildDocxReport(spec: any): Promise<Uint8Array> {
  ...
  toc.forEach((t: string, i: number) => {
    contentChildren.push(new Paragraph({
      ...
      children: [new TextRun({ text: `${i + 1}.  ${t}`, size: HALF(11), color: COLORS.ink, font: FONTS.body })],
    }));
  });
    ...
    if (needsConfirm.length) {
      ...
      contentChildren.push(new Paragraph({
        ...
        children: [new TextRun({
          ...
          size: HALF(10), color: COLORS.warn, font: FONTS.body,
        })],
      }));
      ...
      if (sec.subsections?.length) {
        for (const sub of sec.subsections) {
          contentChildren.push(new Paragraph({
            ...
            children: [new TextRun({ text: sub.heading || '', size: HALF(13), bold: true, color: COLORS.ink, font: FONTS.heading })],
          }));
    ...
    headers: {
      default: new Header({
        children: [new Paragraph({
          ...
          children: [
            new TextRun({ text: spec.company?.name || 'Company', size: HALF(8), color: COLORS.sub, font: FONTS.body }),
            new TextRun({ text: '\t', size: HALF(8) }),
            new TextRun({ text: spec.modelTitle || 'Report', size: HALF(8), color: COLORS.sub, font: FONTS.body }),
          ],
        })],
      }),
    },
    footers: {
      default: new Footer({
        children: [new Paragraph({
          ...
          children: [
            new TextRun({ text: 'CONFIDENTIAL', size: HALF(8), color: COLORS.sub, font: FONTS.body }),
            new TextRun({ text: '\t', size: HALF(8) }),
            new TextRun({ children: [PageNumber.CURRENT], size: HALF(8), color: COLORS.sub, font: FONTS.body }),
            new TextRun({ text: ' / ', size: HALF(8), color: COLORS.sub, font: FONTS.body }),
            new TextRun({ children: [PageNumber.TOTAL_PAGES], size: HALF(8), color: COLORS.sub, font: FONTS.body }),
          ],
        })],
      }),
    },
  ...
  const doc = new Document({
    ...
    styles: {
      default: {
        document: {
          run: { font: FONTS.body, size: HALF(11) },
        },
      },
    },
  });

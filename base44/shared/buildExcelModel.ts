...
const styleHeaderCell = (cell) => {
  ...
  cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF2F3437' } };
...
function buildCover(ws, spec) {
  ...
  ws.getCell('B2').font = { size: 24, bold: false, color: { argb: 'FF2F3437' } };

  ws.mergeCells('B3:C3');
  ws.getCell('B3').value = spec.modelTitle || 'Financial Model';
  ws.getCell('B3').font = { size: 16, color: { argb: 'FF7A5C3E' } };
...
function buildAssumptions(ws, spec) {
  ...
  ws.getCell('A1').font = { size: 16, bold: false, color: { argb: 'FF2F3437' } };
...
function buildExecSummary(ws, spec) {
  ...
  ws.getCell('B2').font = { size: 16, color: { argb: 'FF2F3437' } };
  ...
  if (spec.narrative?.executiveSummary) {
    ...
    ws.getCell(`B${r}`).font = { bold: true, color: { argb: 'FF7A5C3E' } };
    r++;
...
function buildRevenueModel(ws, spec) {
  ...
  ws.getCell('A1').font = { size: 16, color: { argb: 'FF2F3437' } };
